...
const deliveryAreaZipCodes = [85205, 85204, 85203, 85213, 85206];
let zipCheck = false;
function canWeDeliverTwo(zipCheck){
    let zipInZone = false;
    for (let i = 0; i < deliveryAreaZipCodes.length; i++){
        if (zipCheck === deliveryAreaZipCodes[i]){
            zipInZone = true;            
            break;
    }
}
...
*/
const dealsFifteen = deals
console.log(dealsFifteen)
const updatedDealsFifteen = dealsFifteen.replace('15% Off!', "10% Off");
console.log(updatedDealsFifteen);
...
const updatedMonth = dealsFifteen.replace('March', "April");
const monthNoSpace = updatedMonth.trim();
console.log(monthNoSpace);